...
describe Maps::ApiController do 
  ...
  let(:floor_map_response) { Maps::ApiResponse.new(floor_map, mock_request) }
  let(:nil_floor_map_response) { Maps::ApiResponse.new(nil, mock_request) }

  let(:call_number_range) { FactoryGirl.create(:call_number_range) }

  let(:call_number_range_response) { Maps::ApiResponse.new(call_number_range.floor_map, mock_request, call_number) }
  let(:nil_call_number_range_response) { Maps::ApiResponse.new(nil, mock_request, invalid_call_number) }  
