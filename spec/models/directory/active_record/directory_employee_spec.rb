...
describe DirectoryEmployee do
  ...
  describe "Employee Group" do
    ...
    it 'returns one record for a single netid' do
      single_employee = DirectoryEmployee.by_netids([@employee.netid])
      expect(single_employee[0].netid).to eq(@employee.netid)
    end

    it 'returns multiple records if multiple netids are submitted' do
      multiple_employees = DirectoryEmployee.by_netids([@employee.netid, @employee3.netid])
    ...
    it 'returns the correct number of records if all netids match' do
      multiple_employees = DirectoryEmployee.by_netids([@employee.netid, @employee2.netid, @employee3.netid])
      expect(multiple_employees.count).to eq(3)
    end

    it 'only returns records for matched netids' do
      multiple_employees = DirectoryEmployee.by_netids([@employee.netid, 'not-netid', @employee3.netid]) 
