...
describe RefworksPasswordReset do
  it "should create a token" do
    record = FactoryBot.create(:refworks_password_reset)
  ...
  it "should find users by email" do
    ...
    record = FactoryBot.create(:refworks_password_reset, :email_or_login => user.email)
    expect(record.users).to include(user)
  end

  it "should find users by login" do
    user = FactoryBot.create(:refworks_user)
    record = FactoryBot.build(:refworks_password_reset, :email_or_login => user.login)
    expect(record.users).to include(user)
  end
